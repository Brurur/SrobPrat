...
### Distribusi IPM per Wilayah

```{r visualisasi-ipm, fig.cap="Distribusi IPM Berdasarkan Wilayah"}
ggplot(data, aes(x = reorder(Provinsi, IPM), y = IPM, fill = Wilayah)) +
  geom_col() +
  geom_hline(yintercept = mean(data$IPM), linetype = "dashed",
             color = "red", linewidth = 0.8) +
  annotate("text", x = 3, y = mean(data$IPM) + 0.8,
           label = paste("Rata-rata:", round(mean(data$IPM), 2)),
           color = "red", size = 3) +
  coord_flip() +
  scale_fill_brewer(palette = "Set2") +
  labs(title    = "Indeks Pembangunan Manusia (IPM) per Provinsi",
       subtitle = "Garis merah = rata-rata nasional",
       x = "Provinsi", y = "IPM", fill = "Wilayah") +
  theme_minimal(base_size = 9) +
  theme(plot.title    = element_text(face = "bold", hjust = 0.5),
        legend.position = "bottom")
```

### Scatter Plot: IPM vs PDRB per Kapita

```{r scatter-ipm-pdrb, fig.cap="Hubungan IPM dan PDRB per Kapita"}
ggplot(data, aes(x = PDRB_PerKapita, y = IPM, color = Wilayah, label = Provinsi)) +
  geom_point(size = 3, alpha = 0.8) +
  geom_smooth(method = "lm", se = TRUE, color = "black", linetype = "dashed",
              linewidth = 0.7) +
  scale_x_continuous(labels = scales::comma) +
  labs(title    = "Scatter Plot: IPM vs PDRB per Kapita",
       subtitle = "Garis putus-putus = regresi linear",
...
### Perbandingan Kemiskinan dan Elektrifikasi

```{r scatter-kemiskinan-listrik, fig.cap="Kemiskinan vs Rasio Elektrifikasi"}
ggplot(data, aes(x = Elektrifikasi_Pct, y = Kemiskinan_Pct,
                 color = Wilayah, size = IPM)) +
  geom_point(alpha = 0.8) +
  scale_size_continuous(range = c(2, 8)) +
  labs(title    = "Kemiskinan vs Elektrifikasi",
       subtitle = "Ukuran titik = nilai IPM",
       x = "Rasio Elektrifikasi (%)",
